...
/** Verifies FR-007/FR-007a/FR-007b: update creates a new version; history is retrievable. */
class UpdateCredentialIT extends IntegrationTestBase {
  ...
  @Test
  void updateCreatesNewVersionAndHistoryIncludesBoth() {
    HttpHeaders headers = loginAndGetSessionHeaders("update-basic@example.com", "Password1!");

    ResponseEntity<CredentialEntryResponse> created = restTemplate.exchange(
        baseUrl("/credentials"), HttpMethod.POST,
        new HttpEntity<>(new CredentialVersionInput("svc-user", "oldPass", null, null), headers),
        CredentialEntryResponse.class);
    var credentialId = created.getBody().id();

    ResponseEntity<CredentialEntryResponse> updated = restTemplate.exchange(
        baseUrl("/credentials/" + credentialId), HttpMethod.PUT,
        new HttpEntity<>(new CredentialVersionInput("svc-user", "newPass", null, null), headers),
        CredentialEntryResponse.class);

    assertThat(updated.getStatusCode()).isEqualTo(HttpStatus.OK);
    assertThat(updated.getBody().password()).isEqualTo("newPass");
    ...
    assertThat(history.getBody()[0].password()).isEqualTo("oldPass");
    assertThat(history.getBody()[1].password()).isEqualTo("newPass");
  }

  @Test
  void updateRejectedOnceCredentialIsDeleted() {
    HttpHeaders headers = loginAndGetSessionHeaders("update-deleted@example.com", "Password1!");
